...
select ename,job from emp1 where sal>3000 group by ename,job;
select min(hiredate), max(hiredate) from emp1
select deptno, count(*) from emp1 where job='salesman' group by deptno having count(*)>2
select ename,sal,deptno from emp1 where sal>3000